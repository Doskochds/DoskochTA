    ...
    const payload = {
      ...
      questions: data.questions.map((q) => ({
        ...
        options: q.type === 'checkbox' 
          ? q.optionsString.split(',').map((s: string) => s.trim()).filter((s: string) => s !== '')
          : [] 
      }))
    };
    ...
    <div className="max-w-3xl mx-auto p-8 font-sans pb-24">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        <div className="bg-white p-6 rounded-xl border shadow-sm">
          <label className="block font-semibold text-gray-700 mb-2">Quiz Title</label>
          <input 
        ...
        <div className="space-y-6">
          {fields.map((field: any, index: number) => {
            ...
            return (
              <div key={field.id} className="bg-white border border-gray-200 p-6 rounded-xl shadow-sm relative group hover:border-blue-300 transition">
                ...
                <div className="grid gap-5">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1">Answer Type</label>
                    <select 
                  ...
                  {currentType === 'checkbox' && (
                    <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                      ...
                      <input 
                        {...register(`questions.${index}.optionsString`, { required: true })}
                        placeholder="e.g. 1,2,3,4"
                        className="w-full border border-blue-200 p-3 rounded bg-white focus:ring-2 focus:ring-blue-400 outline-none"
                      />
                      <p className="text-xs text-blue-600 mt-1">Users will be able to select multiple options.</p>
                    </div>
                  )}
                  
                  {currentType === 'boolean' && (
                    <div className="text-sm text-gray-500 italic bg-gray-50 p-3 rounded">
                      User will simply select between "True" and "False".
                    </div>
                  )}
                </div>
              </div>
